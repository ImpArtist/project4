...
​        // Generate cache key
​        String cacheKey = CACHE_KEY_PREFIX + table + ":" + attribute + ":" + value + ":" + order + ":" + desc + ":" + start + ":" + count;

​        // Try to get data from Redis
​        String cachedData = redisTemplate.opsForValue().get(cacheKey);
​        if (cachedData != null) {
​            return new ObjectMapper().readValue(cachedData, new TypeReference<List<LinkedHashMap<String, Object>>>() {});
​        }

​        // Query from database
​        List<LinkedHashMap<String, Object>> result = baseMapper.queryListByAttribute(table, attribute, value, select, order, desc, start, count, type);

​        // Save to Redis
​        redisTemplate.opsForValue().set(cacheKey, new ObjectMapper().writeValueAsString(result));
...
private String[] JsonToArrays(String attris) {
    // Implement your logic to convert JSON string to String array
    return new String[0];
}

private String configerSQL(String table, String[] attributes) {
    // Implement your logic to configure SQL based on table and attributes
    return "";
}
